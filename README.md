...
> In this score-based Glicko-2 model, ratings predict **expected match intensity** (win margin) rather than purely win probability. The `scaled_outcome` uses the normalized score margin to indicate how decisively a fencer won a bout, allowing the model to capture both skill and dominance.
